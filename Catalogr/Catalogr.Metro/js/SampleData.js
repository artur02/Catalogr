...
(function (context) {
    WinJS.Namespace.define("SampleData", {
        load: function startLoadData(dbname, version) {
            ...
            dbRequest.onerror = function () { Logger.log && Logger.log("Error opening database.", "sample", "error"); };
            dbRequest.onsuccess = function (evt) { dbSuccess(evt, dbname, version); };
            dbRequest.onupgradeneeded = function (evt) { Logger.log && Logger.log("Database wrong version.", "sample", "error"); if (SdkSample.db) { SdkSample.db.close(); } };
            dbRequest.onblocked = function () { Logger.log && Logger.log("Database access blocked.", "sample", "error"); };
        }
    });    
    ...
    function dbSuccess(evt, dbname, version) {
        ...
        if (context.db.objectStoreNames.length === 0) {
            Logger.log && Logger.log("Database schema does not exist. Complete the first scenario before continuing.", "sample", "error");
            context.db.close();
            context.db = null;
            ...
            Windows.ApplicationModel.Package.current.installedLocation.getFolderAsync(foldername).done(function (folder) {
                ...
                folder.getFileAsync(filename).done(function (file) {
                    ...
                    Windows.Data.Xml.Dom.XmlDocument.loadFromFileAsync(file, loadSettings).done(function (xmlDoc) {
                        ...
                        var txn = context.db.transaction(["books", "authors"], "readwrite");
                        txn.oncomplete = function () {
                            Logger.log && Logger.log("Database populated.", "sample", "status");
                        };
                        txn.onerror = function () {
                            Logger.log && Logger.log("Unable to populate database or database already populated.", "sample", "error");
                        };
                        ...
                        for (i = 0; i < len; i++) {
                            ...
                            addResult.onerror = function () {
                                WinJS && Logger.log("Failed to add book: " + this.book + ".", "sample", "error");
                            };
                        }
                        ...
                        for (i = 0; i < len; i++) {
                            ...
                            addResult.onerror = function () {
                                WinJS && Logger.log("Failed to add author: " + this.author + ".", "sample", "error");
                            };
                        }
                    });
                });
